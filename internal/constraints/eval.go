package constraints

import (
	"regexp"
	"strings"

	"github.com/umitbozkurt/orchestrator/internal/config"
)

type Result struct{ OK bool; Reason string }

func Evaluate(tags map[string]string, clauses []config.ConstraintClause) Result {
	for _, c := range clauses {
		if r := evalOne(tags, c); !r.OK {
			return r
		}
	}
	return Result{OK: true}
}

func evalOne(tags map[string]string, c config.ConstraintClause) Result {
	key := strings.TrimSpace(c.Key)
	op := strings.ToLower(strings.TrimSpace(c.Op))
	v, has := tags[key]

	switch op {
	case "exists":
		if !has {
			return Result{false, "missing tag " + key}
		}
		return Result{true, ""}
	case "not_exists":
		if has {
			return Result{false, "tag exists " + key}
		}
		return Result{true, ""}
	case "equals":
		if !has || v != c.Value {
			return Result{false, key + " != " + c.Value}
		}
		return Result{true, ""}
	case "not_equals":
		if has && v == c.Value {
			return Result{false, key + " == " + c.Value}
		}
		return Result{true, ""}
	case "in":
		if !has {
			return Result{false, "missing tag " + key}
		}
		for _, vv := range c.Values {
			if v == vv {
				return Result{true, ""}
			}
		}
		return Result{false, key + " not in list"}
	case "not_in":
		if !has {
			return Result{true, ""}
		}
		for _, vv := range c.Values {
			if v == vv {
				return Result{false, key + " in blocked list"}
			}
		}
		return Result{true, ""}
	case "matches":
		if !has {
			return Result{false, "missing tag " + key}
		}
		re, err := regexp.Compile(c.Value)
		if err != nil {
			return Result{false, "bad regex"}
		}
		if !re.MatchString(v) {
			return Result{false, key + " regex mismatch"}
		}
		return Result{true, ""}
	default:
		return Result{false, "unknown op " + c.Op}
	}
}
