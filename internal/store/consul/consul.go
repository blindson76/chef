...
func (s *ConsulStore) PutLeased(ctx context.Context, key string, val []byte, ttl time.Duration) (store.LeaseHandle, error) {
	...
	if !ok {
		_ = s.destroySession(context.Background(), sessID)
		return nil, fmt.Errorf("acquire failed for %s", key)
	}
...
func (s *ConsulStore) Put(ctx context.Context, key string, val []byte) (uint64, error) {
	...
	if !ok {
		return 0, fmt.Errorf("put succeeded but key not found: %s", key)
	}
...
func (s *ConsulStore) PutCAS(ctx context.Context, key string, val []byte, expectedRev uint64) (uint64, bool, error) {
	p := &capi.KVPair{Key: key, Value: val, ModifyIndex: expectedRev}
	ok, _, err := s.c.KV().CAS(p, nil)
	if err != nil {
		return 0, false, err
	}
	if !ok {
		return expectedRev, false, nil
	}
	v, vok, err := s.Get(ctx, key)
	if err != nil {
		return 0, true, err
	}
	if !vok {
		return 0, true, fmt.Errorf("CAS succeeded but key not found: %s", key)
	}
	return v.Revision, true, nil
}
...
func (s *ConsulStore) List(ctx context.Context, prefix string) ([]store.Value, error) {
	...
	for _, p := range pairs {
		...
		out = append(out, store.Value{Key: p.Key, Data: p.Value, Revision: uint64(p.ModifyIndex), SessionID: p.Session})
	}
	return out, nil
}
	...
	go func() {
		...
		for {
			...
			for _, p := range pairs {
				...
				ev := store.Event{Type: store.EventPut, Value: store.Value{Key: p.Key, Data: p.Value, Revision: uint64(p.ModifyIndex), SessionID: p.Session}}
				select {
				case ch <- ev:
				default:
				}
...
func (s *ConsulStore) createSession(ctx context.Context, ttl time.Duration) (string, error) {
	se := &capi.SessionEntry{Behavior: capi.SessionBehaviorDelete, TTL: ttl.String(), LockDelay: 0}
