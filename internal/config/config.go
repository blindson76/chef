...
type KafkaConfig struct {
    ...
    StorageFormatBinary string  `yaml:"storageFormatBinary"`
...
func applyEnvOverrides(c *Config) {
    ...
    for _, e := range os.Environ() {
        ...
        key := strings.TrimPrefix(parts[0], "ORCH_TAG_")
...
func (cl ConstraintClause) String() string {
    if len(cl.Values) > 0 {
        return fmt.Sprintf("%s %s %v", cl.Key, cl.Op, cl.Values)
    }
    return fmt.Sprintf("%s %s %s", cl.Key, cl.Op, cl.Value)
}
