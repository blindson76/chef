...
func InspectOffline(ctx context.Context, d Dirs) (*model.KafkaOfflineStatus, error) {
    ...
    // meta.properties may exist under meta dir
    ...
    // check cluster metadata dir presence
    // KRaft metadata typically under metadata log dir as __cluster_metadata-0 or similar
    ...
    // read-only artifacts heuristic: any *.deleted or file attribute is hard; best-effort by name.
    ro := false
    filepath.WalkDir(d.MetaDir, func(p string, de os.DirEntry, err error) error {
        if err != nil || de == nil {
            return nil
        }
